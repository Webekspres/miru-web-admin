...
export function PublicAbout() {
  const { data, error, isLoading, mutate } = useSWR(
    ...
    () => api.get<InstitutionSettings>('/settings/', undefined, { skipAuth: true }),
  )
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="max-w-3xl text-3xl font-extrabold tracking-tight sm:text-4xl">
                Tentang MIRU Bank Sampah
              </h1>
              <p className="mt-3 max-w-2xl text-sm leading-relaxed text-white/85 sm:text-base">
                MIRU (Mimika Recycle Unit) — aplikasi bank sampah resmi Distrik
                Mimika Baru: sampah bernilai, lingkungan bersih, warga sejahtera.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          {isLoading ? (
            ...
            <ErrorMessage
              ...
              message="Tidak dapat mengambil informasi tentang MIRU. Periksa koneksi lalu coba lagi."
              onRetry={() => mutate()}
            />
          ) : (
            <article className="rounded-xl bg-white p-6 shadow-[0_18px_50px_-32px_rgba(6,78,59,0.55)] ring-1 ring-emerald-900/5 sm:p-8">
              <MarkdownContent source={data?.tentang ?? '_Belum ada konten._'} />
            </article>
          )}
